...
/* ---- Escrita profissional (gerar / corrigir) ---- */
export async function writingHelp({ action, docType, text }) {
  const res = await fetch("/api/ai", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ mode: "writing", action, docType, text }),
  });
  if (!res.ok) throw new Error("writing_failed");
  return res.json();
}

/* ---- Tradução sob demanda (EN -> PT) ---- */
